...
public class Budget
{
    ...
    private static void buttonCreate() {
        ...
        addExpense = new JButton("Add Monthly Expense");

        // if edit income button is clicked...
        editIncome.addActionListener(e -> {
            String stringInc = JOptionPane.showInputDialog(frame, "Enter new monthly income", null);
    ...
    private static void setTextFields()
    {
        ...
        for (int i = 0; i < itemList.size(); i++)
        {
            ...
            {
                linkedItemList.add(0, "0.0");
                linkedItemList.add(0, "income");
            }

            // item is income, grab value as double.
            else if (linkedItemList.get(i).equals("income") && (i + 1) < linkedItemList.size())
            {
                income = Double.parseDouble(linkedItemList.get(i + 1));
                showIncome.setText("$ " + income);
            }
    ...
    private static void updateFileInfo()
    {
        ...
        {
            // if next expense is 0, continue.
            if (i != (splitComponents.length - 1))
            {
                if (splitComponents[i + 1].equals("0") || splitComponents[i + 1].equals("0.0") || splitComponents[i + 1].equals("") || splitComponents.equals("\\s+"))
                {
                    i++;
                    continue;
                }
            ...
            newLine++;
            newFileInput += splitComponents[i];
    ...
    // grabs current directory and the expenses.txt file.
    public static void grabFile()
    {
        fileName = System.getProperty("user.dir") + "\\expenses.txt";
